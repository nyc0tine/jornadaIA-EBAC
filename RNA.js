...
class RNA {
    constructor(inputCount = 1, levelList = []) {
        ...
        this.levelList = levelList.map((l, i) => {
            const inputSize = i === 0 ? inputCount : levelList[i - 1] // operador ternário = ? 

            return new Array(l).fill().map(() => new Neuron(inputSize)) // map sempre recebe uma callback
        })
    }
